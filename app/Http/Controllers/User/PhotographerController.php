...
class PhotographerController extends Controller
{
    ...
    public function photographer_details($id){
        ...
        $gellary = Gallery::orderBy('galleryID','DESC')->where('photographerID',$id)->get();
        return view('user.photographer_details')->with(['table' => $table, 'count' => $count,'review' => $review,'gellary' => $gellary]);
    }
    ...
    public function remove_cmnt($id){
        $comnt = PhotographerReview::find($id);
        $comnt->delete();
        return redirect()->back();
    }
}
