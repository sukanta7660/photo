    ...
    <div class="row">
      <div class="col-sm-5">
        ...
        <div class="port-info">
          ...
          @php $totalRating = $table->ratingTotal($table->photographerID); $totalUser = $table->totalUser($table->photographerID); if ($totalRating > '0' ){ $grandRating = $totalRating/$totalUser; } else{ $grandRating = 0; } @endphp
          <form action="#">
            <fieldset class="rate">
              ...
              <span class="fa fa-star checked"></span>
              <span class="fa fa-star checked"></span>
              <span class="fa fa-star checked"></span> ( {{number_format($grandRating,2)}} )
            </fieldset>
          </form>
        </div>
      </div>
      <div class="col-sm-7">
        ...
        <div class="row">
          <div class="col-lg-12">
            <div class="section-title pt-4 text-center">
              <h2>Portfolio Photos</h2>
            </div>
          </div>
          @foreach($gellary as $row)
          <div class="col-lg-4 col-md-6">
              <a href="{{asset('public/uploads/gallery/'.$row->images)}}" class="image-popup">
                  <img src="{{asset('public/uploads/gallery/'.$row->images)}}" alt="">
              </a>
          </div>
           @endforeach
        </div>
    ...
    <div class="row">
      <div class="col-sm-12">
        <div class="port-rating-area">
          ...
          {{--@else--}} {{--
          <h4 class="text-muted mt-2 mb-2 text-center">You already reviewed <i class="fa fa-smile-o"></i></h4>--}} {{--@endif--}}
          <div class="comnt">
          ...
          <div class="row mb-4">
            ...
            <div class="col-sm-10">
              <div class="new-cmmnt">
                <h5>&apos;{{$row->user['name']}}&apos; Commented on</h5>
                <h6>{{$row->comment}}</h6>
                  @if(isset(Auth::user()->email))
                @if($row->userID == Auth::user()->id)
                <a onclick="return confirm('Sure to remove this comment?')" href="{{action('User\PhotographerController@remove_cmnt',['id' => $row->photographerReviewID])}}">Remove</a>
                  @endif
                      @endif
              </div>
            </div>
